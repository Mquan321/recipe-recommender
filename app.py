...
.stApp {{ {f'background-image: url("data:image/jpg;base64,{bg_img}");' if bg_img else ''}
    background-size: cover; background-position: center; background-attachment: fixed; }}
.app-overlay {{ position: fixed; inset: 0; background: linear-gradient(135deg, rgba(255,255,255,0.75) 0%, rgba(245,247,250,0.8) 100%);
    z-index: 0; pointer-events: none; }}
.reportview-container .main {{ padding: 2rem 1.5rem; position: relative; z-index: 1; }}
.main-header {{
    background: linear-gradient(135deg, #667eea 0%, #764ba2 50%, #f093fb 100%);
    padding: 2rem 1.5rem; border-radius: var(--card-radius); text-align: center;
    box-shadow: var(--shadow); color: white; z-index: 2; margin-bottom: 1.75rem;
    border: 1px solid rgba(255,255,255,0.2);
}}
.main-header h1 {{ margin: 0; font-size: 2.5rem; font-weight: 800; letter-spacing: -1.2px; text-shadow: 0 2px 12px rgba(0,0,0,0.15); }}
.main-header .subtitle {{ font-size: 1.05rem; margin-top: 0.5rem; color: rgba(255,255,255,0.95); font-weight: 500; }}
...
.section-header {{ background: linear-gradient(135deg, rgba(102,126,234,0.08), rgba(118,75,162,0.08));
    padding: 1.25rem 1.5rem; border-radius: 12px; margin: 1.75rem 0 1.25rem 0;
    border-left: 4px solid var(--accent-1); box-shadow: 0 2px 8px rgba(0,0,0,0.04); }}
.section-header h2 {{ margin: 0; font-size: 1.75rem; font-weight: 700; color: var(--accent-contrast); }}
.section-header h3 {{ margin: 0; font-size: 1.4rem; font-weight: 700; color: var(--accent-contrast); }}
...
.eda-card img {{ width: 100%; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); }}
...
.recipe-title {{ font-weight: 700; color: var(--accent-contrast); font-size: 1.05rem; }}
...
.footer {{ text-align: center; margin-top: 2.5rem; padding: 1.5rem; color: var(--muted);
    background: var(--card-bg); border-radius: var(--card-radius); border: 1px solid var(--card-border); box-shadow: var(--shadow); }}
@media (max-width: 768px) {{ .eda-container {{ grid-template-columns: 1fr; }} .stat-grid {{ grid-template-columns: repeat(2, 1fr); }} }}
</style>
    ...
    if st.button("Recommend Top-20 Recipes", type="primary", use_container_width=True):
        ...
        with colm1:
            ...
            <div style='background: linear-gradient(135deg, rgba(102,126,234,0.08), rgba(118,75,162,0.08));
                ...
                <div class='stat' style='margin: 0;'><div class='label'>RMSE</div><div class='value'>{rmse}</div></div>
                <div class='stat' style='margin: 0;'><div class='label'>R²</div><div class='value'>{r2}</div></div>
            </div>
            """, unsafe_allow_html=True)
        with colm2:
            st.markdown(f"""
            <div style='background: linear-gradient(135deg, rgba(255,107,107,0.08), rgba(255,142,83,0.08));
                ...
                <div style='display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.75rem;'>
                    ...
                    <div class='stat' style='margin:0;'><div class='label'>mAP@20</div><div class='value'>{map20}</div></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
